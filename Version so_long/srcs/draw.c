...
/*   Updated: 2024/10/14 16:38:30 by carzhang         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	copy_pixels_to_mlx_image(t_mlix *mlx, t_imgs *img_src, int x_start,
		int y_start)
{
	int		x;
	int		y;
...
{
	if (tile == '1')
		copy_pixels_to_mlx_image(mlx, &mlx->wall, i, j);
	else if (tile == 'C')
		copy_pixels_to_mlx_image(mlx, &mlx->col, i, j);
	else if (tile == 'E')
		copy_pixels_to_mlx_image(mlx, &mlx->exit, i, j);
	else if (tile == 'P')
		copy_pixels_to_mlx_image(mlx, &mlx->player, i, j);
}
...
void	fill_main_image_with_differents_tiles(t_imgs *img_dst, t_mlix *mlx)
{
	...
	while (j < img_dst->height && mlx->map.grid[j])
	{
		...
		while (i < img_dst->width && mlx->map.grid[j][i])
		{
			...
			copy_pixels_to_mlx_image(mlx, &mlx->sol, i * TILESIZE, j
				* TILESIZE);
