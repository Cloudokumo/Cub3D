...
/*   Updated: 2024/10/16 11:52:03 by carzhang         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_map(t_mlix *mlx)
{
		...
		&& mlx->pos_player.y >= 0 && mlx->pos_player.y < mlx->map.height)
	{
		...
		copy_pixels_to_mlx_image(mlx, &mlx->sol, mlx->pos_player.x * TILESIZE,
		...
		&& mlx->new_pos_player.y < mlx->map.height)
	{
		...
		copy_pixels_to_mlx_image(mlx, &mlx->player, mlx->new_pos_player.x
			* TILESIZE, mlx->new_pos_player.y * TILESIZE);
	}
